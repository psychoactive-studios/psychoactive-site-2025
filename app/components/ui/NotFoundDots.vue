  ...
  try {
    ...
    const paths = pathElements
      .map((el) => {
        ...
        const rawFillRule = (el.getAttribute('fill-rule') || '').toLowerCase();
    ...
    console.error('NotFoundDots: Error loading SVG:', url, e);
  }
}
...
function updateShapeVisibility(animate = true) {
  ...
  // Temporarily reset transform for isPointInPath checks
  if (svgData.value) {
    ctx.save();
